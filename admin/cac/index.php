...
if($palavra)
{
	$oCAC->SQLWhere .= " AND (Titulo LIKE '%" . $palavra . "%' OR Descricao LIKE '%" . $palavra . "%') ";
}

$oPaginator = new Paginator($oCAC->GetCount());
$oCAC->SQLOrder = "Ordem DESC";
$oCAC->LoadByPaginator($oPaginator->Limit, $oPaginator->Total);
...
if($oPaginator->TotalRecords > 0)
{
	...
	<table class="lista">
		...
		<tbody>
			...
			{
				$Param = "?" . $oPaginator->ParameterWithPage . "&id=" . $oCAC->ID;
				?>
				<tr>
					<td><?=$oCAC->Titulo;?></td>
					<td align="center">
