...
$oAudienciaPublica->SQLWhere = " 1 = 1 ";

$palavra = $_GET["palavra"];
if($palavra)
{
	$oAudienciaPublica->SQLWhere .= " AND (Titulo LIKE '%" . $palavra . "%' OR Descricao LIKE '%" . $palavra . "%') ";
}

$oPaginator = new Paginator($oAudienciaPublica->GetCount());
$oAudienciaPublica->SQLOrder = "Data DESC";
$oAudienciaPublica->LoadByPaginator($oPaginator->Limit, $oPaginator->Total);
...
if($oPaginator->TotalRecords > 0)
{
	...
	<table class="lista">
		...
		<tbody>
			<?php
			for($c = 0; $c < $oAudienciaPublica->NumRows; $c++)
			{
				$Param = "?" . $oPaginator->ParameterWithPage . "&id=" . $oAudienciaPublica->ID;
				?>
				<tr>
					<td align="center"><?=date("d/m/Y", $oAudienciaPublica->DateShow($oAudienciaPublica->Data));?></td>
					<td><?=$oAudienciaPublica->Titulo;?></td>
					<td align="center">
