...
$oVereadorExterno->SQLWhere = " 1 = 1 ";

$palavra = $_GET["palavra"];
if($palavra)
{
	$oVereadorExterno->SQLWhere .= " AND (Titulo LIKE '%" . $palavra . "%' OR Descricao LIKE '%" . $palavra . "%') ";
}

$oPaginator = new Paginator($oVereadorExterno->GetCount());
$oVereadorExterno->SQLOrder = "Ordem DESC";
$oVereadorExterno->LoadByPaginator($oPaginator->Limit, $oPaginator->Total);
...
if($oPaginator->TotalRecords > 0)
{
	...
	<table class="lista">
		...
		<tbody>
			...
			{
				$Param = "?" . $oPaginator->ParameterWithPage . "&id=" . $oVereadorExterno->ID;
				?>
				<tr>
					<td><?=$oVereadorExterno->Titulo;?></td>
					<td align="center">
