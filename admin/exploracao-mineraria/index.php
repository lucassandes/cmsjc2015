...
$oExploracaoMineraria->SQLWhere = " 1 = 1 ";

$palavra = $_GET["palavra"];
$tipo = $_GET["tipo"];
if($palavra)
{
	$oExploracaoMineraria->SQLWhere .= " AND (Titulo LIKE '%" . $palavra . "%' OR Descricao LIKE '%" . $palavra . "%') ";
}
if($tipo)
{
	$oExploracaoMineraria->SQLWhere .= " AND Tipo = '" . $tipo . "' ";
}

$oPaginator = new Paginator($oExploracaoMineraria->GetCount());
$oExploracaoMineraria->SQLOrder = "Data DESC";
$oExploracaoMineraria->LoadByPaginator($oPaginator->Limit, $oPaginator->Total);
...
if($oPaginator->TotalRecords > 0)
{
	...
	<table class="lista">
		...
		<tbody>
			<?php
			for($c = 0; $c < $oExploracaoMineraria->NumRows; $c++)
			{
				$Param = "?" . $oPaginator->ParameterWithPage . "&id=" . $oExploracaoMineraria->ID;
				?>
				<tr>
					<td align="center"><?=date("d/m/Y", $oExploracaoMineraria->DateShow($oExploracaoMineraria->Data));?></td>
					<td><?=$oExploracaoMineraria->Titulo;?></td>
					<td align="center">
