...
class graph
{
	...
	*/
	...
	function esc( $text )
	{
		...
		return urlencode( utf8_encode($tmp) );
	}
	...
	*/
	...
	function set_is_time( $val )
	{
		$this->is_time = $val?'true':'false';
	}
	...
	{
		// TO DO escape commas:
		$this->links[] = implode(',',$this->esc($links));
	}
	...
	function pie_values( $values, $labels=array(), $links=array() )
	{
		$labels_format = array();
		$links_format = array();
		foreach($labels as $c) { array_push($labels_format, $this->esc($c)); }
		foreach($links as $c) { array_push($links_format, $this->esc($c)); }
	...
	function render()
	{
		...
		if( strlen( $this->is_time ) > 0 )
			$tmp[] = $this->format_output('is_time',$this->is_time);
